...
├── docs/invariants.md                 ← project invariants and conventions
...
A short doc capturing the hard invariants and conventions every change to
the pipeline must respect. Lives at [`docs/invariants.md`](./invariants.md);
covers partition-key=PK, commit-after-side-effect, LSN-gated writes,
producer settings, RF=3 / ISR=2, DLQ write-only, plus Go/metrics/test
conventions and the "verification before claiming done" checklist.
